package oit.is.team63.nsk3.teamsixthree.security;
...
public class Sample3AuthConfiguration {
  ...
  public InMemoryUserDetailsManager userDetailsService() {
    ...
    UserDetails shiro = User.withUsername("shiro")
        .password("{bcrypt}$2y$05$cHbSF9p54xvxo9LGM8zup.ws3bgSSD1Ay96e1xvF.mK4w/IG0e9tK").roles("DOG").build();
    UserDetails futa = User.withUsername("futa")
        .password("{bcrypt}$2y$05$JBBjYbAvrRfwbcVvxy7TZuAOQZY.kf4FphuiCPWNt44zJ46dkiAF.").roles("CAT").build();
    UserDetails admin = User.withUsername("admin")
        .password("{bcrypt}$2y$10$ngxCDmuVK1TaGchiYQfJ1OAKkd64IH6skGsNw1sLabrTICOHPxC0e").roles("ADMIN").build();

    // 生成したユーザをImMemoryUserDetailsManagerに渡す（いくつでも良い）
    return new InMemoryUserDetailsManager(shiro, futa, admin);
  }
